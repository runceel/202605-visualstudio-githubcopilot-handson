...
上記が終わって時間が残っている場合は、次の項目を試してみてください。

| 項目 | 内容 |
|------|------|
| 画面での目視確認 | Aspire AppHost を起動し、実際のブラウザ画面で表示を確認する |
| レビュー向け説明 | 変更内容と確認結果を Agent に整理させ、PR 説明文の下書きを作る |
| おまけを読む | このリポジトリの Copilot カスタマイズ（カスタム指示・スキル・エージェント）の仕組みを確認する |
...
GitHub のツールが適切に構成されている場合は、GitHub Copilot からそのままプルリクエストを作成することもできます。
...
スキルを使うことで、「この作業ではどの手順やルールに従うべきか」を Copilot に明示できます。チームで開発する場合、設計ルール、テスト方針、PR 作成手順などをスキルとして整備しておくと、作業の進め方を揃えやすくなります。
...
個人で Copilot を使う場合は、プロンプトにその都度指示を書く方法でも問題ありません。チーム開発では、次のような情報をリポジトリに置いておくことで、Copilot の回答や実装方針をメンバー間で揃えやすくなります。
...
この `.github` 配下の定義は、コード生成だけでなく、チームの開発プロセスや設計ルールを Copilot と共有するための一例です。
